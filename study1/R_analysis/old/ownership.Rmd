...
emb_df %>%
  group_by(condition) %>%
  get_summary_stats(owner_ave, type = "mean_sd")
...
# ggplot(emb_df,aes(x=order,y=owner_ave, fill=order))+geom_boxplot(aes(fill=order)) + labs( x = "Order", y = "owner_ave", fill = "Order") +   theme_minimal() +
...
# 
# ######################### cool dot plot #########################
# ggplot(emb_df,aes(x=order,y=owner_ave, fill=order)) + geom_boxplot() + geom_dotplot(binaxis='y', stackdir='center', stackratio=1.5, dotsize=1.2) +
#                     labs( x = "Order", y = "owner_ave", fill = "Order") + theme_minimal() +
# # stat_summary(fun = mean, geom = "point",size = 3, shape = 2, position = position_dodge(0.75), show.legend = FALSE) +
# scale_fill_brewer(palette = "Set2") +
#     theme(plot.title = element_text(size = 15, face = "bold"),
#         axis.title.x = element_text(size = 14, face = "bold"),
...
# 
# ######################### cool dot plot 2 ######################### 
# p <- ggplot(emb_df,aes(x=order,y=owner_ave, fill=order)) + geom_dotplot(binaxis='y', stackdir='center', stackratio=1.5, dotsize=0.6) + 
...
emb_df %>%
  group_by(condition) %>%
  identify_outliers(owner_ave)

######################### check normality ######################### 
emb_df %>%
  group_by(condition) %>%
  shapiro_test(owner_ave)
ggqqplot(emb_df, "owner_ave", facet.by = "condition")
...
pwc <- emb_df %>%
  pairwise_t_test(
    owner_ave ~ condition, paired = TRUE,
...
# pwc <- emb_df %>%
#   group_by(condition) %>%
#   pairwise_t_test(owner_ave ~ order, p.adjust.method = "bonferroni")
# pwc
# 
# ######################### NON-SIG 2-WAY INTER: Pairwise comparisons between group levels #########################
# emb_df %>%
#   pairwise_t_test(
#     owner_ave ~ autonomy, paired = TRUE,
#     p.adjust.method = "bonferroni"
#   )
# 
# emb_df %>%
